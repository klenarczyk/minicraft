...
/// <summary>
/// A headless renderer that captures 3D block models into 2D sprites.
/// Used to generate inventory icons dynamically at startup.
/// </summary>
public sealed class IconGenerator : IDisposable
{
    ...
    public IconGenerator(Shader iconShader)
    {
        ...
        // --- Setup Framebuffer (Offscreen Render Target) ---
        ...
        // Depth Attachment (Required for correct occlusion)
    ...
    /// <summary>
    /// Renders a specific block to the internal FBO and returns the pixels as an Image.
    /// </summary>
    /// <param name="debugName">Optional: Saves the icon to disk for debugging.</param>
    public Image<Rgba32> GenerateBlockIcon(Texture2D blockAtlas, Block block, string? debugName = null)
    {
        ...
        // Setup MVP Matrix
        var model = Matrix4.CreateTranslation(-0.5f, -0.5f, -0.5f); // Center block at origin
        var cameraPos = new Vector3(2.0f, 1.7f, 2.0f); // Standard isometric angle
        var view = Matrix4.LookAt(cameraPos, Vector3.Zero, Vector3.UnitY);
        var projection = Matrix4.CreateOrthographic(1.9f, 1.9f, 0.1f, 10.0f); // Ortho ensures no perspective distortion
        ...
        // OpenGL reads pixels bottom-up, ImageSharp expects top-down
    ...
    private void RenderCube(Block block)
    {
        ...
        foreach (var face in facesToDraw)
        {
            ...
            {
                // Fallback: Use the first available texture if specific face is missing
    ...
    private static void RenderFace(BlockFace face, Vector4 uvs)
    {
        ...
        // Dynamic vertex data generation
        ...
        // Layout: 0:Pos(3), 1:UV(2), 2:Normal(3)
        // Stride: 8 floats
